...
//!
//! The AccountLinker pallet provides functionality for linking a WinDEX account to account at
//! other networks. (currently support Ethereum (BSC), BTC and Polkadot ecosystem)
//!
//! ## Overview
//!
//! The AccountLinker pallet stores the linking relation between WinDEX accounts and accounts at other
...
//!
//! * `link_eth` - Link an Ethereum address to a WinDEX account providing a proof signature
//! from the private key of that Ethereum address.
//! * `link_btc` - Link an BTC address to a WinDEX account providing a proof signature
//! from the private key of that BTC address.
//! * `link_polkadot` - Initiate a link request to link a WinDEX address to another WinDEX address
//! * `accept_polkadot` - Accept a pending `link_polkadot` request to link a WinDEX address
//! to another WinDEX address.
//!
//! [`Call`]: ./enum.Call.html
...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		/// Link an Ethereum address to the origin WinDEX account providing a proof signature from the private key
		...
		/// - `addr_expected`: The intended Ethereum address to link to the origin's WinDEX address
		...
		/// Link a BTC address to a WinDEX account providing a proof signature from the private key
		...
		///
		/// Parameters:
		/// - `account`: The WinDEX address that is to be linked
		/// - `index`: The index of the linked BTC address that the user wants to replace with.
		/// - `addr_expected`: The intended BTC address to link to the origin's WinDEX address
		/// - `expiring_block_number`: The block number after which this link request will expire
		/// - `sig`: The rsv-signature generated by the private key of the addr_expected
		///
		...
		/// Initiate a link request to link a WinDEX address (= any account in Polkadot ecosystem)
		/// to another WinDEX address (= any account in Polkadot ecosystem).
		///
		/// Parameters:
		/// - `account`: The WinDEX address that is to be linked
		/// - `index`: The index of the linked WinDEX address that the user wants to replace with.
		...
		/// Accept a pending `link_polkadot` request to link a WinDEX address (= any account in Polkadot ecosystem)
		/// to another WinDEX address (= any account in Polkadot ecosystem).
		///
		/// The runtime needs to ensure that a malicious index can be handled correctly.
		/// Currently, when vec.len > MAX_ETH_LINKS, replacement will always happen at the final index.
	...
	impl<T: Config> Pallet<T> {
		...
		/// Format: "Link WinDEX: " + WinDEX account + expiring block number
		fn generate_raw_message(
			account: &T::AccountId,
			expiring_block_number: T::BlockNumber,
		) -> Vec<u8> {
			let mut bytes = b"Link WinDEX: ".encode();
